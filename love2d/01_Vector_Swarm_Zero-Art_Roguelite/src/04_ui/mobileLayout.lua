...
function mobileLayout.updateScreenInfo()
    ...
    -- 영역별 크기 계산
    screenInfo.topAreaHeight = math.floor(screenInfo.screenHeight * UI_TOP_RATIO)
    screenInfo.playAreaStart = screenInfo.topAreaHeight
    screenInfo.playAreaHeight = math.floor(screenInfo.screenHeight * PLAY_AREA_RATIO)
...
function mobileLayout.calculateDPIScale()
    ...
    local diagonal = math.sqrt(screenInfo.screenWidth^2 + screenInfo.screenHeight^2)
    local estimatedDPI = diagonal / 6  -- 6인치 가정
    return math.max(0.8, math.min(2.0, estimatedDPI / referenceDPI))
end
...
function mobileLayout.getSafeTouchSize(baseSize)
    ...
    return math.max(baseSize * layout.touchScale, minTouchSize)
end

-- 디바이스 프로필 반환
function mobileLayout.getDeviceProfile()
    local layout = mobileLayout.getLayout()
    local diagonal = math.sqrt(layout.screenWidth^2 + layout.screenHeight^2)
